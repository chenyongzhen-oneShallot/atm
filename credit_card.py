...
from conf import templates, errorcode
...
def user_login(userobj, today, weekoftoday):
    ...
    while not quitflag:
        if userobj.islogin:
            ...
            print(templates.index_user_center.format(userobj.name, today, common.numtochr(weekoftoday)))
            _chooseflag = False
            ...
            if _choose == "6":
                ...
                func_dict = templates.user_center_func
...
def card_center(userobj):
    ...
    show_template = templates.index_ATM
    quitflag = False
    while not quitflag:
        ...
        # 查看信用卡信息
        if _choose == "1":
            common.show_message(templates.card_info.format(cardno=card.cardno,
                                                          owner=card.owner,
...
def get_users():
    ...
    if _deluser.load_user_info():
        # 先显示一下用户的信息
        common.show_message(templates.user_info.format(username=_deluser.username,
                                                      name=_deluser.name,
...
def manager(userobj):
    ...
    if userobj.islogin:
        if userobj.role == "admin":
            quit_flag = False
            while not quit_flag:
                _show_template = templates.index_admin
...
if __name__ == "__main__":
    ...
    while not exitflag:
        ...
        if not curruser.islogin:
            print(templates.index_default_menu.format("", today, common.numtochr(weekoftoday)))
        else:
            print(templates.index_logined_menu.format("欢迎您: {0}".format(curruser.name), today,
